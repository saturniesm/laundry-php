...
<body>
    <?php
        ...
        $sql = "SELECT transaction.*, package.type, package.price, user.username, member.name as member_name, member.address, member.tlp, detail_transaction.qty
                FROM transaction
                JOIN user ON user.id_user = transaction.id_user
                JOIN member ON member.id_member = transaction.id_member
                JOIN detail_transaction ON detail_transaction.id_transaction = transaction.id_transaction
                JOIN package ON package.id_package = detail_transaction.id_package
                WHERE transaction.id_transaction = '$tr_id'";
                //echo $sql;
    ...
    ?>
		<header>
			...
			<address >
				<p>Member name :<?php echo $data['member_name']; ?></p>
		...
		<article>
			<h1>Recipient</h1>
			<address >
				<p>DETAIL TRANSACTION LAUNDRY</p>
			</address>
			<table class="meta">
				<tr>
					<th><span >Transaction Id</span></th>
					<td><span ><?php echo $data['id_transaction']; ?></span></td>
				</tr>
				<tr>
